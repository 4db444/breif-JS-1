...
let page_items = 9;
let page_number = 1;
let liked_cards = cards.filter(elem => elem.is_liked)
let filtered_cards = [...liked_cards]
...
function toggleLike (id){
    ...
    liked_cards = liked_cards.filter(elem => elem.id !== id)
    filtered_cards = filtered_cards.filter(elem => elem.id !== id)

    if(liked_cards.length <= 9) page_number = 1;
...
document.querySelector("form").addEventListener("change", (e)=> {
    ...
    if (filter_by === "all"){
        filtered_cards = [...liked_cards]
        renderCards(filtered_cards)
    }else {
        filtered_cards = liked_cards.filter(elem => elem.rarity === filter_by)
        renderCards(filtered_cards)
    }
